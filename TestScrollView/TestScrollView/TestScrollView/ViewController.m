...
- (void)viewDidLoad {
    ...
    [self.view addSubview:_mPhotoScrollView];
...
//    UIView *inView = gesture.view;
//    CGPoint anchorPoint;
//    if (gesture.numberOfTouches == 2) { //两指缩放的情况
//        CGPoint onePoint = [gesture locationOfTouch:0 inView:inView];
//        CGPoint twoPoint = [gesture locationOfTouch:1 inView:inView];
//        anchorPoint.x = (onePoint.x + twoPoint.x) / 2 / inView.bounds.size.width;
//        anchorPoint.y = (onePoint.y + twoPoint.y) / 2 / inView.bounds.size.height;
//        if (self.currentZoomScale >= self.maximumZoomScale) {
//            self.passMaxScaleAnchorPoint = anchorPoint;
//        }
//    } else {    //双击缩放情况
//        
//        
...
//    _cropperImage.frame=CGRectMake(_cropperImage.frame.origin.x+event.accessibilityActivationPoint.x, _cropperImage.frame.origin.y+event.accessibilityActivationPoint.y, _cropperImage.frame.size.width, _cropperImage.frame.size.height);
...
//    
//    //移动前的中点位置
//    CGPoint center=_cropperImage.center;
//    //移动偏移量
//    CGPoint offset=CGPointMake(current.x-previous.x, current.y-previous.y);
//    
//    //重新设置新位置
//    _cropperImage.center=CGPointMake(center.x+offset.x, center.y+offset.y);
...
//    CGFloat centerOfX = (CGRectGetMidX(self.landscapePPTView.frame) - CGRectGetMinX(self.normalLandscapeImageFrame))/CGRectGetWidth(self.normalLandscapeImageFrame);
//    CGFloat centerOfY = (CGRectGetMidY(self.landscapePPTView.frame) - CGRectGetMinY(self.normalLandscapeImageFrame))/CGRectGetHeight(self.normalLandscapeImageFrame);
//    [self.socketDelegate sendPPTMove:PSO_MOVE centerPoint:CGPointMake(centerOfX, centerOfY)];
